...
def fastaTOprediction(stepSize, X, Y):
    ...
    def evaluateModel(X_test):
      ...
      [2.54285714, 2.96625007]])
        ...
        filename1 = os.path.join(this_folder,'iRecModelEF.pkl')
    ...
    for x, y in zip(X, Y):
        ...
        for i in range(0, len(x), stepSize):
            ...
            if len(x) < end:
                sequencesNumber.append((begin, len(x)))
            else:
            ...
            sequences.append(x[i:i + stepSize])
            ...
            itype, probability = evaluateModel(X_test)

            types.append(itype)
            probabilities.append(probability)

        SequencesNumber.append(sequencesNumber)
        Types.append(types)
        Probabilities.append(probabilities)
        Sequences.append(sequences)

    return SequencesNumber, Types, Probabilities, Sequences
